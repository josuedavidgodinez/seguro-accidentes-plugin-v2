...
define('SACAIG_NAME_EMPRESA', 'Tres Mares Correduría');
define('SACAIG_URLEMPRESA', 'https://tresmares.com');
define('SACAIG_MAIL_EMPRESA', 'carlos.calderon@tresmares.es');
define('SACAIG_LOGO_TOP_EMPRESA', 'https://tresmares.com/wp-content/uploads/2023/10/logo-tresmares-corto.svg');
define('SACAIG_DIRECCION_EMPRESA', 'Ronda de Nelle 12 - A Coruña - 15167');
define('SACAIG_TELEFONO_EMPRESA', '981 27 21 87');
define('SACAIG_TELEFONO_EMPRESA_2', '658 950 377');
define('SACAIG_CODIGO_MEDIADOR', 5);



// VALORES A DEFINIR ANTES DE ACTIVAR EL PLUGIN
...
/******* ESTA FUNCIÓN GENERA LAS PÁGINAS NECESARIAS PARA EL FUNCIONAMIENTO DEL PLUGIN **********/
function SACAIG_crear_paginas() {

   // Listado de páginas del plugin
   if (!file_exists(SACAIG_PLUGIN_PATH . 'utils/paginas-crear-caracteristicas.php')) {
      error_log('El archivo paginas-crear-caracteristicas.php no se encontró.');
      return;
   }
   ...
   foreach ($SACAIG_paginas as $pagina) {
      ...
      if (empty($pagina['slug']) || empty($pagina['title'])) continue;
      ...
      if (!$pagina_existente) {
         ...
         if (is_wp_error($id)) {
               error_log('Error creando la página: ' . $id->get_error_message());
               continue;
         }

         // Añadir meta datos con Rank Math
         if (isset($pagina['meta_title'])) update_post_meta($id, 'rank_math_title', sanitize_text_field($pagina['meta_title']));
         if (isset($pagina['meta_description'])) update_post_meta($id, 'rank_math_description', sanitize_text_field($pagina['meta_description']));
         if (isset($pagina['indexable'])) update_post_meta($id, 'rank_math_robots', $pagina['indexable'] ? 'index' : 'noindex');
         if (isset($pagina['visible_in_search'])) update_post_meta($id, 'exclude_from_search', !$pagina['visible_in_search']);
      }
   }
}

register_activation_hook(__FILE__, 'SACAIG_crear_paginas');
...
function SACAIG_eliminar_paginas() {
    ...
    foreach ($page_slugs as $slug) {
        ...
        if ($pagina) {
            ...
            if (!$result) error_log('No se pudo eliminar la página con slug: ' . $slug);
        }
    }
}

register_uninstall_hook(__FILE__, 'SACAIG_eliminar_paginas');
...
function SACAIG_custom_page_template($template) {
   ...
   foreach ($page_slugs as $slug) {
      if ($post->post_name == $slug) {
         ...
         return SACAIG_PLUGIN_PATH . 'templates/' . $slug . '.php';
      }
   }
...
add_filter('template_include', 'SACAIG_custom_page_template');
...
function SACAIG_check_required_page() {
   ...
   foreach ($SACAIG_paginas as $pagina) {
      if ($current_slug === $pagina['slug']) return true;
   }
...
/******** AJAX PROCESA LA GENERACIÓN DEL PROECTO, CUMPLIMENTACIÓN DE LA PÓILIZA Y SU FIRMA ************/
function SACAIG_procesar_poliza() {
   ...
   if (!isset($_POST['poliza']) || empty($_POST['poliza'])) {
      ...
      $apellidos_asegurado = $_POST['apellidos'];
   ...
   $response = ['url_proyecto'=> $url_proyecto_producto, 'respuesta_firma' => $firmaResponse];
...
function SACAIG_EnvioCorreoPolizaCompania($email_asegurado, $link_poliza) {
   ...
   require_once SACAIG_PLUGIN_PATH .'templates/template-mail-compania.php';
